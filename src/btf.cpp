...
std::unique_ptr<std::istream> BTF::get_all_traceable_funcs(
    const util::KernelFunctionInfo &kernel_func_info) const
{
  if (!all_funcs_.empty()) {
...
ast::Pass CreateParseBTFPass()
{
  return ast::Pass::create(
      ...
      [](ast::ASTContext &ast,
         BPFtrace &b,
         ast::FunctionInfo &func_info_state) {
        // Create ProbeMatcher locally for module listing.
        ProbeMatcher probe_matcher(&b,
                                   func_info_state.kernel_function_info(),
                                   func_info_state.user_function_info());
